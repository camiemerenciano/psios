...
import { Building2, Plus, X, Loader2, TrendingUp, TrendingDown } from 'lucide-react'
...
export default function ConsultorioPage() {
  ...
  async function loadData() {
    ...
    if (!user) return
    const { data } = await supabase.from('financial_transactions').select('*').eq('psychologist_id', user.id).eq('category', 'consultorio').order('date', { ascending: false })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(false)
    setShowForm(false)
    setForm({ type: 'receita', description: '', amount: '', date: new Date().toISOString().split('T')[0], payment_method: 'pix', status: 'confirmado', notes: '' })
    loadData()
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-foreground">Financeiro · Consultório</h1>
          <p className="text-sm text-muted-foreground mt-0.5">Receitas e despesas do consultório</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="rounded-xl bg-card border border-border p-4">
          <p className="text-xs text-muted-foreground mb-2">Receitas</p>
          <p className="text-xl font-bold text-emerald-400">{formatCurrency(receitas)}</p>
        </div>
        <div className="rounded-xl bg-card border border-border p-4">
          <p className="text-xs text-muted-foreground mb-2">Despesas</p>
        ...
        <div className="rounded-xl bg-card border border-border p-4">
          ...
          <p className={`text-xl font-bold ${saldo >= 0 ? 'text-emerald-400' : 'text-destructive'}`}>{formatCurrency(saldo)}</p>
        </div>
      </div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="rounded-xl bg-card border border-border p-5 space-y-4">
          <h2 className="text-sm font-semibold text-foreground border-b border-border pb-3">Novo lançamento</h2>
              ...
              return (
                <div key={t.id} className="flex items-center gap-4 px-5 py-3.5">
                  ...
                  <div className="text-right shrink-0">
                    <p className={`text-sm font-semibold ${tp?.color}`}>
